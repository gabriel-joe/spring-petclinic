pipeline {
	...
	environment {
	  ...
	  APP_VERSION = sh(""" echo -e "export APP_VERSION_2=\$(xmllint --xpath '/*[local-name()="project"]/*[local-name()="version"]/text()' pom.xml)" >> variables && source variables """)
	}
    stages {
        stage('Build') {
            steps {
                echo 'Clean/Build'
				sh 'echo $APP_VERSION_2'
